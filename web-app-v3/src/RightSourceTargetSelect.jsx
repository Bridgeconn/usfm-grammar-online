...
export default function RightSourceTargetSelect({
  onChange,
  source,
  onTabChange,
}) {
  ...
  const handleFormatChange = (newValue) => {
    ...
    onChange(newValue);
    // Call the tab change handler to trigger conversion
    if (onTabChange) {
      onTabChange(newValue.name);
    }
  };

  return (
    <div className="ml-2 w-56">
      <Listbox value={selected} onChange={handleFormatChange}>
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <span className="block truncate">{selected ? selected.name : "Select format"}</span>
